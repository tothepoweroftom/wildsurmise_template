...
WILDSURMISE is a ~~template~~ lifestyle for creating and building JUCE plugins in 2023.
...
* [ ] Replace `WILDSURMISE` with the name of your project in `CMakeLists.txt` where the `PROJECT_NAME` variable is first set. Make this all one word, no spaces. 
...
* [ ] Build n' Run! If you want to generate an Xcode project, run `cmake -B Builds -G Xcode`. Or just open the project in CLion or VS2022. Running the standalone might be easiest, but you can also build the `AudioPluginHost` that comes with JUCE. Out of the box, WILDSURMISE's VST3/AU targets should already be pointing to it's built location.
